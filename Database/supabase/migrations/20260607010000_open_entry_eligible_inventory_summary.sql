...
as $$
  ...
  last_prize_counts as (
    select
      ...
      end as last_prize_eligible_units
    from round_inventory ri
    join public.draw_rounds dr on dr.id = ri.draw_round_id
  )
  select coalesce(jsonb_agg(jsonb_build_object(
    'drawRoundId', ri.draw_round_id,
    'totalSlots', ri.total_slots,
    'remainingSlots', ri.remaining_slots,
    'claimedSlots', ri.claimed_slots,
    'voidSlots', ri.void_slots,
    'totalUnits', coalesce(uc.total_units, 0),
    'availableUnits', coalesce(uc.available_units, 0),
    'availableWinSlots', coalesce(nwc.available_win_slots, 0) + coalesce(lpc.last_prize_eligible_units, 0),
    'eligibleUnits', coalesce(nwc.eligible_win_slots, 0) + coalesce(lpc.last_prize_eligible_units, 0),
    'awardedUnits', coalesce(uc.awarded_units, 0),
    'reservedUnits', coalesce(uc.reserved_units, 0),
    'voidUnits', coalesce(uc.void_units, 0)
  ) order by ri.sort_order, ri.created_at desc), '[]'::jsonb)
  from round_inventory ri
