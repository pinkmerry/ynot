-- Edit a LIVE random pack in place and re-materialize its prize stock safely.
--
-- The campaign stays live/public/approved throughout. The whole operation runs
-- in one transaction and FIRST takes `select ... for update` on the draw_rounds
-- row. Because open_gacha_campaign holds that same row lock for its entire
-- transaction, this serializes the edit against every concurrent customer open,
-- so no open can be mid-flight and none can start until the edit commits — this
-- eliminates the award race.
--
-- Reconciliation is delta-based, never the draft path's blunt delete+reinsert:
--   * awarded units are preserved verbatim (a customer already won them);
--   * only `available` units are released back to stock for decreases;
--   * increases reserve+allocate+materialize just the additional units;
--   * total_slots is forced to equal the new planned total and draw_slots are
--     grown/shrunk to match (never below already-consumed slots).
--
-- Hard rejects (owner-approved rules):
--   * cannot reduce a prize/slots below its already-awarded count;
--   * cannot remove a prize that has awarded units (hide it instead);
--   * cannot change a prize's sub-SKU identity once it has awarded units.
--
-- p_prizes is the full desired prize list (the API builds each row's metadata
-- exactly like saveInitialPrizes): a jsonb array of objects with keys
--   cardId, tier, rank, valueThb, convertCoinValue, weight, unlockAtSoldPct,
--   plannedQuantity, isTest, seedRunId, metadata
...
begin
  ...
  -- 2. Stage the desired prize list.
  ...
  -- 4. Reconcile each desired prize. Guards run against the EXISTING row before
  --    the upsert overwrites it. Target units = planned for visible prizes, or
  --    the awarded count for hidden prizes (so awards survive and the prize
  --    leaves the live pool). v_planned_total accumulates the per-prize targets.
  for desired in select * from _desired_prizes order by ord loop
    ...
    if desired.hidden then
      v_delta := v_awarded; -- target = awarded
    else
    ...
    v_delta := v_delta - v_nonvoid; -- delta to apply
  ...
  -- 6. Sync total_slots + draw_slots to the new planned total. create_draw_slots
  --     reads total_slots from the round, so set it first; it only adds the
  --     missing high-numbered slots. Then trim surplus available slots.
  ...
  -- 11. Re-assert the publish invariants; any mismatch rolls the tx back.
  select
...
-- Internal helper: reserve+allocate+materialize p_count fresh units for a prize.
...
begin
  ...
  ins_units as (
    ...
    select p_draw_round_id, p_prize_id, alloc.card_id, alloc.id, 'available', p_seed_run_id,
...
-- Internal helper: void + release up to p_count available units of a prize
-- (or all available units when p_count is null). Awarded units are never
-- touched. Returns the number released.
create or replace function public._release_live_prize_units(
  p_draw_round_id uuid,
  p_prize_id uuid,
  p_admin_id uuid,
  p_count integer
...
begin
  with victims as (
    select u.id as unit_id, u.card_stock_unit_id
    from public.draw_round_prize_units u
    where u.draw_round_prize_id = p_prize_id
      and u.draw_round_id = p_draw_round_id
  ...
  void_units as (
    update public.draw_round_prize_units u
    set status = 'void',
        voided_at = coalesce(u.voided_at, now()),
        metadata = u.metadata || jsonb_build_object('reason', 'live_edited', 'voidedByAdminId', p_admin_id)
    from victims
    where u.id = victims.unit_id
  ...
  free_stock as (
    ...
    set status = 'available',
        ...
        metadata = s.metadata || jsonb_build_object('releasedByAdminId', p_admin_id, 'reason', 'live_edited'),
        updated_at = now()
    from victims
    where s.id = victims.card_stock_unit_id
  ...
  rel_resv as (
    ...
    set status = 'released',
        ...
        released_at = now(),
        metadata = r.metadata || jsonb_build_object('reason', 'live_edited')
    from victims
    where r.stock_unit_id = victims.card_stock_unit_id
  ...
  led as (
    ...
    select fs.id, fs.card_id, p_draw_round_id, p_prize_id, 'reservation_released', p_admin_id,
           jsonb_build_object('reason', 'live_edited')
    from free_stock fs
    returning 1
  )
  select count(*)::integer into v_removed from void_units;
...
revoke all on function public._materialize_live_prize_units(uuid, uuid, uuid, integer, uuid, text, jsonb) from public, anon, authenticated;
revoke all on function public._release_live_prize_units(uuid, uuid, uuid, integer) from public, anon, authenticated;
