-- Pack opening reveal assets + richer open_gacha_campaign result payload.
--
-- Adds a small admin-editable table for the 4 tier reveal animations
-- (rainbow / gold / silver / bronze) plus a public Storage bucket for the
-- video files, and extends open_gacha_campaign to return card name, image,
-- and displayTier so the reveal overlay can render without a second fetch.
-- Also rehydrates result.items on idempotent replay (previously empty).
--
-- Production apply remains gated by Database/README.md backup/PITR rules.

create table if not exists public.tier_animations (
  tier text primary key check (tier in ('bronze', 'silver', 'gold', 'rainbow')),
  video_url text,
  poster_url text,
  sound_url text,
  duration_ms integer not null default 2500 check (duration_ms between 500 and 15000),
...
begin
  if exists (
    ...
    drop trigger if exists tier_animations_touch_updated_at on public.tier_animations;
    create trigger tier_animations_touch_updated_at
      before update on public.tier_animations
      for each row execute function app_private.touch_updated_at();
  end if;
end $$;

insert into public.tier_animations (tier, duration_ms, is_active) values
  ('bronze', 1500, true),
  ('silver', 2000, true),
  ('gold',   3000, true),
  ('rainbow', 4500, true)
...
alter table public.tier_animations enable row level security;
revoke all on table public.tier_animations from public, anon, authenticated;
grant select on table public.tier_animations to anon, authenticated;
grant all on table public.tier_animations to service_role;

drop policy if exists "Anyone can read active tier animations" on public.tier_animations;
create policy "Anyone can read active tier animations"
on public.tier_animations
for select
to anon, authenticated
using (is_active = true);

-- Storage bucket for the actual video / poster / audio files. Admin uploads
-- via the dashboard; client reads via the public URL stored on the row.
...
values (
  ...
  array['video/mp4', 'video/webm', 'image/png', 'image/jpeg', 'image/webp', 'audio/mpeg', 'audio/mp3', 'audio/wav', 'audio/ogg']
)
...
create policy "Public can read tier animation assets"
on storage.objects
for select
to anon, authenticated
using (bucket_id = 'tier-animations');

-- Recreate open_gacha_campaign so the reveal screen has everything it needs.
-- Adds card name / imageUrl / displayTier to each result item, and rehydrates
-- items on idempotent replay (so a retried POST still drives the animation).
