-- Make the owner-selected random logic mode authoritative.
--
-- Modes:
-- - pure_random: all non-hidden available prize units are equal chance.
-- - weighted_templates: unlock gates are ignored; positive weight controls odds.
-- - inventory_gated: sold unlock gates apply first, then positive weight controls odds.
--
-- The draw loop recalculates sold percentage after each slot opens so large
-- multi-open orders can cross an unlock checkpoint during the same request.
...
using (
  ...
  and exists (
    ...
    where dr.id = draw_round_prizes.draw_round_id
      ...
      and (
        coalesce(dr.logic_snapshot->>'mode', 'pure_random') = 'pure_random'
        or coalesce(draw_round_prizes.weight, 1) > 0
      )
      and (
        coalesce(dr.logic_snapshot->>'mode', 'pure_random') <> 'inventory_gated'
        or coalesce(draw_round_prizes.unlock_at_sold_pct, 0) <= (
          select
            case
