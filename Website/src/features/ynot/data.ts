...
type MonitorOpenRow = Pick<
  Database["public"]["Tables"]["gacha_opens"]["Row"],
  "id" | "public_code" | "profile_id" | "opened_at" | "created_at"
>;

type MonitorProfileRow = Pick<
  Database["public"]["Tables"]["profiles"]["Row"],
  "id" | "email" | "display_name" | "line_display_name" | "line_user_id"
>;
...
function monitorProfileLabel(profile?: MonitorProfileRow) {
  return (
    profile?.display_name ??
    profile?.line_display_name ??
    profile?.email ??
...
function monitorRemainingStatus(status: MonitorPrizeUnitRow["status"]) {
  return status === "available" || status === "reserved";
}

function monitorOutStatus(status: MonitorPrizeUnitRow["status"]) {
...
export async function getAdminPackMonitor(
  ...
  if (!viewer.isAdmin && !isDevAuthAllowed()) return null;
  ...
  const supabase = createServiceSupabaseClient();
  const rows = await readOrEmpty("admin_pack_monitor", async () => {
    const prizes = await readSupabaseRows<MonitorPrizeRow>(
      "admin_pack_monitor_prizes",
      () =>
        supabase
          .from("draw_round_prizes")
          .select("*")
    ...
    const cardIds = [...new Set(visiblePrizes.map((prize) => prize.card_id))];
    const [cards, prizeUnits, prizeImageByPrizeId] = await Promise.all([
      cardIds.length
        ...
        : Promise.resolve([]),
      prizeIds.length
        ? readSupabaseRows<MonitorPrizeUnitRow>(
            "admin_pack_monitor_units",
            () =>
              supabase
                .from("draw_round_prize_units")
                .select(
                  "draw_round_prize_id,status,profile_id,gacha_open_id,awarded_at,updated_at",
                )
    ...
    const unitsByPrizeId = new Map<string, MonitorPrizeUnitRow[]>();
    for (const unit of prizeUnits) {
      const group = unitsByPrizeId.get(unit.draw_round_prize_id) ?? [];
      group.push(unit);
      unitsByPrizeId.set(unit.draw_round_prize_id, group);
    }
    const displayedWinnerUnits = [...unitsByPrizeId.values()].flatMap((units) =>
      units
        .filter((unit) => monitorOutStatus(unit.status))
        .sort(
          (left, right) =>
            new Date(right.awarded_at ?? right.updated_at ?? 0).valueOf() -
            new Date(left.awarded_at ?? left.updated_at ?? 0).valueOf(),
        )
        .slice(0, ADMIN_PACK_MONITOR_WINNERS_PER_PRIZE_LIMIT),
    );
    const displayedWinnerUnitSet = new Set(displayedWinnerUnits);

    const openIds = [
      ...new Set(
        displayedWinnerUnits
          .map((unit) => unit.gacha_open_id)
          .filter((id): id is string => Boolean(id)),
      ),
    ];
    const opens = openIds.length
      ? await readSupabaseRows<MonitorOpenRow>("admin_pack_monitor_opens", () =>
          supabase
            .from("gacha_opens")
            .select("id,public_code,profile_id,opened_at,created_at")
            .in("id", openIds),
        )
      : [];
    ...
    const profileIds = [
      ...new Set(
        [
          ...displayedWinnerUnits.map((unit) => unit.profile_id),
          ...opens.map((open) => open.profile_id),
        ].filter((id): id is string => Boolean(id)),
      ),
    ];
    const profiles = profileIds.length
      ? await readSupabaseRows<MonitorProfileRow>(
          "admin_pack_monitor_profiles",
          () =>
            supabase
              .from("profiles")
              .select("id,email,display_name,line_display_name,line_user_id")
              .in("id", profileIds),
        )
      : [];
    ...
    const monitorPrizes = visiblePrizes
      .map((prize) => {
        const card = cardById.get(prize.card_id);
        const counts = plannedPrizeUnitCounts(prize);
        const units = unitsByPrizeId.get(prize.id) ?? [];
        const hasUnits = units.length > 0;
        const totalUnits = hasUnits ? units.length : counts.total;
        const remainingUnits = hasUnits
          ? units.filter((unit) => monitorRemainingStatus(unit.status)).length
          : counts.available;
        const outUnits = hasUnits
          ? units.filter((unit) => monitorOutStatus(unit.status)).length
          : counts.awarded;
        const winners = units
          .filter((unit) => monitorOutStatus(unit.status))
          .filter((unit) => displayedWinnerUnitSet.has(unit))
          .map((unit) => {
            const open = unit.gacha_open_id
              ? openById.get(unit.gacha_open_id)
              : undefined;
            const profile = profileById.get(
              unit.profile_id ?? open?.profile_id ?? "",
            );
            return {
              status: unit.status,
              openedAt: unit.awarded_at ?? open?.opened_at ?? unit.updated_at,
              ownerLabel: monitorProfileLabel(profile),
              ownerEmail: profile?.email ?? null,
              ownerLineUserId: profile?.line_user_id ?? null,
              publicOpenCode: open?.public_code ?? null,
            };
          })
          .sort(
            (left, right) =>
              new Date(right.openedAt ?? 0).valueOf() -
              new Date(left.openedAt ?? 0).valueOf(),
          );
        const displayTier = displayTierFromPrizeMetadata(prize);
        return {
          id: prize.id,
    ...
    const updatedAt = monitorLatestTimestamp(
      campaign.createdAt,
      ...prizeUnits.map((unit) => unit.awarded_at ?? unit.updated_at),
      ...opens.map((open) => open.opened_at ?? open.created_at),
    );
