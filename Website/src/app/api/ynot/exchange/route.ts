...
export async function POST(request: Request) {
  ...
  const { data, error } = await supabase.rpc("submit_exchange_order", { p_profile_id: session.profileId, p_collection_item_ids: collectionItemIds, p_customer_note: note, p_idempotency_key: idempotencyKey });
