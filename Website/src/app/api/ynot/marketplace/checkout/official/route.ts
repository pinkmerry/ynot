...
import { createOfficialPendingPaymentOrder } from "@/lib/marketplace/orders";
...
export async function POST(request: Request) {
  const mutation = await prepareMarketplaceMutation(request, {
    ...
    action: "checkout",
    rateLimit: {
      key: "ynot:marketplace:checkout:official",
      limit: 8,
      windowMs: 60_000,
    },
  ...
  try {
    ...
    const order = await createOfficialPendingPaymentOrder({
