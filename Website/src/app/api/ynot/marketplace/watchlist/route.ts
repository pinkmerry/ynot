...
import {
  getMarketplaceWatchlistState,
  toMarketplacePublicWatchlistState,
...
export async function GET(request: Request) {
  ...
  const rateLimited = await enforceRateLimit(
    request,
    "ynot:marketplace:watchlist:list",
  ...
  try {
    ...
    const state = await getMarketplaceWatchlistState(account, profile.profileId);
    const payload = toMarketplacePublicWatchlistState(state);
    return Response.json({
      ok: true,
