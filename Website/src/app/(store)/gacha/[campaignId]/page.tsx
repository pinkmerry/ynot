...
export default async function GachaDetailPage({ params }: { params: Promise<{ campaignId: string }> }) {
  ...
  return (
    <YnotShell viewer={data.viewer}>
      <PageHeader eyebrow="02 · Gacha Detail" title={campaign?.titleTh ?? "Campaign not found"} description="Pre-pull · S/A/B/C/D tiers · stock count · provably fair." action={campaign ? <Link className="primary-action" href={`/gacha/${campaign.slug}/open`}>Pull now</Link> : undefined} />
