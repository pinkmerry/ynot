...
check("src/app/globals.css", "admin campaign builder keeps pack info and top prizes horizontal on desktop", /admin-pack-info-panel \.admin-form-grid[\s\S]*grid-template-columns: repeat\(4, minmax\(0, 1fr\)\)[\s\S]*admin-top-prize-strip[\s\S]*grid-template-columns: repeat\(3, minmax\(0, 1fr\)\)[\s\S]*@media \(max-width: 720px\)[\s\S]*admin-top-prize-strip[\s\S]*grid-template-columns: 1fr/);
...
check("src/features/ynot/client.tsx", "owner approval queue exposes owner-only value weight unlock odds panel", /owner-prize-odds-panel[\s\S]*Owner-only prize odds[\s\S]*Value, weight, and sold unlock[\s\S]*saveOwnerPrizeOdds/);
