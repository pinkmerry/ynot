...
check("src/app/globals.css", "admin campaign horizontal builder has desktop and mobile layout CSS", /admin-pack-builder-layout[\s\S]*grid-template-columns:[\s\S]*admin-readiness-panel[\s\S]*position: sticky[\s\S]*@media \(max-width: 720px\)[\s\S]*admin-pack-builder-layout[\s\S]*grid-template-columns: 1fr/);
check("src/app/globals.css", "admin campaign horizontal builder spans studio width and stacks top prize controls safely", /admin-page-grid-studio > \.admin-pack-form-horizontal[\s\S]*grid-column: 1 \/ -1[\s\S]*admin-top-prize-controls[\s\S]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\)[\s\S]*admin-top-prize-controls \.admin-field:first-child[\s\S]*grid-column: 1 \/ -1/);
