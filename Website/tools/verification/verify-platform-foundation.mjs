...
check("src/app/(store)/packs/page.tsx", "packs page reads series and tag filters from URL search params", /validSeriesParams[\s\S]*seriesParam[\s\S]*initialSeries[\s\S]*tagParam[\s\S]*initialTag[\s\S]*<YPackExperience/);
...
check("src/features/ynot/cr/YPackExperience.tsx", "CR packs view applies URL-driven series and tag filters", /initialTag[\s\S]*resolveInitialTag\(initialTag[\s\S]*campaignHasTag/);
...
check("src/features/ynot/client.tsx", "bronze PSA10 uses random PSA10 while higher tiers use specific cards", /function prizeCatalogCardsFor[\s\S]*canPrizeDisplayTierUseRandomPsa10[\s\S]*filter\(isRandomPsa10Card\)[\s\S]*filter\(\s*\(card\) => !isRandomPsa10Card\(card\)[\s\S]*Random PSA10 is available for Single Cards/);
check("src/app/api/ynot/admin/campaigns/route.ts", "admin campaign API enforces PSA10 item against display tier", /isRandomPsa10PrizeCard[\s\S]*canPrizeDisplayTierUseRandomPsa10[\s\S]*Random PSA10 can only be used on Bronze/);
check("src/app/api/ynot/admin/prizes/route.ts", "admin prize API enforces PSA10 item against display tier", /isRandomPsa10PrizeCard[\s\S]*canPrizeDisplayTierUseRandomPsa10[\s\S]*Random PSA10 can only be used on Bronze/);
...
check("src/features/ynot/client.tsx", "admin campaign prize rows show selected card image in rank tile", /admin-prize-rank-cell[\s\S]*AdminPrizeCardImage[\s\S]*AdminPrizeCardPicker[\s\S]*showPreview=\{false\}[\s\S]*testIdPrefix=\{`campaign-prize-\$\{prize\.localId\}`\}/);
...
check("src/features/ynot/admin-card-catalog-helpers.ts", "card catalog reader maps every card from the catalog list", /export function buildAdminCardCatalogRows[\s\S]*cards[\s\S]*catalogCardId/);
check("src/features/ynot/admin/prize-catalog/PrizeCatalogScreen.tsx", "admin prize catalog screen renders every card from catalog reader", /buildAdminCardCatalogRows\(cards, prizes\)[\s\S]*pcx-ledger[\s\S]*visible\.map\(/);
...
check("src/app/admin/prizes/page.tsx", "admin prizes page mounts the prize catalog screen from catalog readers", /getAdminCards[\s\S]*getAdminPrizePool[\s\S]*<PrizeCatalogScreen[\s\S]*cards=\{cards\}[\s\S]*prizes=\{prizes\}/);
check("src/features/ynot/admin/prize-catalog/MainSkuForm.tsx", "admin Main SKU create form is upload-backed", /uploadCardImage/);
