...
check("src/features/ynot/client.tsx", "admin campaign action panel uses approval workflow actions", /CampaignWorkflowActions[\s\S]*Save spin config/);
...
check("src/app/api/ynot/admin/campaigns/route.ts", "admin campaign API is admin gated", /requireAdminOrOwner[\s\S]*authErrorResponse/);
...
check("src/app/api/ynot/admin/prizes/route.ts", "admin prize API assigns draw_round_prizes", /requireAdminOrOwner[\s\S]*from\("draw_round_prizes"\)[\s\S]*onConflict: "draw_round_id,tier,rank"/);
