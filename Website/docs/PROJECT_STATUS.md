...
- Production deployment is consolidated under the single GitHub repo `https://github.com/pinkmerry/lucky-draw-liff`. The originally-separate `ynot-lucky-draw-platform` GitHub repo was archived; both Vercel projects (`ynot-lucky-draw-platform` for `www.ynottcg.com` and `lucky-draw-liff` for `liff.ynottcg.com`) now connect to `lucky-draw-liff`, branch `main`, root directory `Website`. Stable fallback aliases: `https://ynot-lucky-draw-platform.vercel.app` (website) and `https://lucky-draw-liff.vercel.app` (LIFF). Evidence: `docs/verification/2026-05-07-github-vercel-production-deploy.md`.
