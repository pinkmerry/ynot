...
const profileSelect = "line_display_name,full_name,phone,address_line1,address_line2,subdistrict,district,province,postal_code,country,delivery_note";
...
async function readProfile() {
  ...
  if (!session?.profileId) {
    return jsonNoStore({ error: "LINE login is required." }, { status: 401 });
  }
  ...
  return jsonNoStore({
    displayName: data.line_display_name ?? session.displayName ?? "LINE Customer",
    profile: toProfileInfo(data),
  });
}
